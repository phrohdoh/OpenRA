...
{
	public class JascPalToGplPalCommand : IUtilityCommand
	{
		string IUtilityCommand.Name => "--jasc-to-gpl";
		...
		[Desc("in.pal [out.gpl OR --to-stdout]", "Converts a JASC palette to a GIMP palette.")]
		void IUtilityCommand.Run(ModData modData, string[] args)
		{
			...
			var colors = JascPalReader.FromFile(a.InputFilename);
			if (colors.Length == 0)
			{
			...
			GplPalWriter.ToTextWriter(colors, a.ShouldSendToStdout ? Console.Out : sw, Path.GetFileName(outputFilename));
