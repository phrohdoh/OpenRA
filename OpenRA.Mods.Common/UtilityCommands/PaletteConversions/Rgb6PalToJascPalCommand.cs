...
{
	public class Rgb6PalToJascPalCommand : IUtilityCommand
	{
		string IUtilityCommand.Name => "--rgb6-to-jasc";
		...
		[Desc("in.pal [out.pal OR --to-stdout]", "Converts a 6bit VGA palette to a JASC palette.")]
		void IUtilityCommand.Run(ModData modData, string[] args)
		{
			var outputFilename = a.OutputFilename ?? Path.GetFileNameWithoutExtension(a.InputFilename) + ".pal";
