...
{
	public class PaletteFromGplInfo : ITraitInfo
	{
		...
		public object Create(ActorInitializer init) { return new PaletteFromGpl(this); }
	}

	public class PaletteFromGpl : ILoadsPalettes, IProvidesAssetBrowserPalettes
	{
		readonly PaletteFromGplInfo info;
		public PaletteFromGpl(PaletteFromGplInfo info) { this.info = info; }
		...
		void ILoadsPalettes.LoadPalettes(WorldRenderer wr)
		{
			...
			uint[] colors;
			if (!GplPalReader.FromLines(lines, out colors))
				return;
